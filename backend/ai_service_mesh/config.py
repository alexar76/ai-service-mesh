...
class Settings(BaseSettings):
    ...
    # Largest agent/hub reply the mesh will buffer. httpx's .json() reads the whole body
    # first, so without this an endpoint could answer a $0.01 task with a gigabyte and the
    # cap would only bite after the memory was already spent (the hub has the same guard).
