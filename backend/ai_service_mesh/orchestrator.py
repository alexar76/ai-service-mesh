...
class MeshOrchestrator:
    def __init__(
        ...
        *,
        ...
        # Threaded into every outbound call: the SSRF re-check + DNS pin needs to know
        # whether the operator allowed localhost agents, and the reply cap is one number
        # for the whole mesh.
        self._allow_localhost = allow_localhost_agents
    ...
    async def run_task(self, task: TaskOut, preferred: list[str]) -> TaskOut:
        ...
        for match in candidates:
            ...
            # Guarantee the hold is always settled (released or refunded). Without this,
            # an unexpected error between hold and settlement (e.g. a store/telemetry
            # failure) would leave the escrow stuck in "held" forever.
            settled = False
            try:
                self._store.emit(
                    ActivityKind.ESCROW,
    ...
    async def _preflight(self, match: DiscoveryMatch) -> tuple[bool, int, str]:
        ...
        return await preflight_agent(
            match.agent.endpoint_url, allow_localhost=self._allow_localhost
        )
