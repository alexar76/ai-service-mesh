...
def client_ip(request: Request, *, trust_forwarded: bool = False) -> str:
    ...
    X-Forwarded-For is attacker-controlled unless the app sits behind a trusted
    reverse proxy that overwrites it. Only honor it when explicitly enabled
    (MESH_TRUST_FORWARDED_FOR), otherwise rate limiting can be trivially bypassed
    by spoofing the header (SEC-08).
    """
    if trust_forwarded:
