...
**File:** [`docker-compose.yml`](docker-compose.yml#L27-L28)
...
**File:** [`security.py`](backend/ai_service_mesh/security.py#L58-L64)
...
**File:** [`security.py`](backend/ai_service_mesh/security.py#L78-L91)
...
**File:** [`orchestrator.py`](backend/ai_service_mesh/orchestrator.py#L36-L158)
...
**File:** [`orchestrator.py`](backend/ai_service_mesh/orchestrator.py#L160-L163)
...
**File:** [`orchestrator.py`](backend/ai_service_mesh/orchestrator.py#L165-L186)
...
**File:** [`App.tsx`](frontend/src/App.tsx#L35-L39)
...
**File:** [`db.py`](backend/ai_service_mesh/db.py#L96-L104)
...
**File:** [`frontend/Dockerfile`](frontend/Dockerfile#L8-L10)
...
**File:** [`orchestrator.py`](backend/ai_service_mesh/orchestrator.py#L55-L56)
...
**File:** [`conftest.py`](backend/tests/conftest.py#L52-L63)
...
**File:** [`App.tsx`](frontend/src/App.tsx#L44)
...
**File:** [`config.py`](backend/ai_service_mesh/config.py#L11)
...
### L3 — `MeshTopology` is purely decorative — **RESOLVED**

The SVG visualization used fixed positions, with "Mesh Core" always centred regardless of
which agents were connected: a hardcoded star, misleading as a "topology" view.

Fixed in `1f408bd3` — `MeshTopology.tsx` was replaced by
[`MeshLiveScene.tsx`](frontend/src/components/MeshLiveScene.tsx), which is driven by the real
agent roster and task hops.

### L4 — No structured logging

All logging uses `logging.info()` / `logging.warning()` with f-strings. In production, structured JSON logging is expected for SIEM ingestion (as mentioned in [deployment.md](docs/deployment.md#L38): "Structured logs from uvicorn (JSON formatter recommended)"). The application itself doesn't output JSON logs.

### L5 — `stats()` method could be expensive under load

**File:** [`db.py`](backend/ai_service_mesh/db.py#L340-L372)
...
**File:** [`payments.py`](backend/ai_service_mesh/payments.py#L1-L33)
...
**File:** [`invoke.py`](backend/ai_service_mesh/invoke.py#L80-L97)
...
**File:** [`api.py`](backend/ai_service_mesh/api.py#L200-L211)
...
**File:** [`api.py`](backend/ai_service_mesh/api.py#L79-L83)
...
**File:** [`vite.config.ts`](frontend/vite.config.ts#L8-L11)
...
1. **SSRF protection** — [`url_is_safe()`](backend/ai_service_mesh/security.py#L40-L65) performs DNS-resolving IP checks against private/loopback/link-local/reserved ranges, blocks `metadata.google.internal`, strips null bytes, and enforces HTTPS. This is a thorough implementation.

2. **Fail-closed CORS** — Empty `MESH_CORS_ORIGINS` means no cross-origin access. Origins must be explicitly configured.

3. **Constant-time token comparison** — Uses [`hmac.compare_digest`](backend/ai_service_mesh/security.py#L74) for Bearer token verification, preventing timing attacks.
...
7. **Demo output rejection** — [`response_is_demo_marked()`](backend/ai_service_mesh/invoke.py#L11-L22) prevents demo/factory executions from being treated as real paid invocations.
